...
static ItemRegistry* reg = NULL;

void item_registry_init() {
	reg = calloc(ITEM_COUNT, sizeof(ItemRegistry));
	if (reg == NULL) {
		TraceLog(LOG_ERROR, "Could not allocate memory for the item registry.");
		return;
	};
	
	reg[ITEM_NONE] = (ItemRegistry){
		.name = "No Item",
	...
	reg[ITEM_GRASS_BLOCK] = (ItemRegistry){
		.name = "Grass Block",
		.atlas_idx = ATLAS_GRASS_BLOCK,
		.model_idx = BLOCK_MODEL_QUAD,
	...
	reg[ITEM_DIRT_BLOCK] = (ItemRegistry){
		.name = "Dirt Block",
	...
	reg[ITEM_SAND_BLOCK] = (ItemRegistry){
		.name = "Sand Block",
	...
	reg[ITEM_STONE_BLOCK] = (ItemRegistry){
		.name = "Stone Block",
	...
	reg[ITEM_COBBLESTONE] = (ItemRegistry){
		.name = "Cobblestone",
	...
	reg[ITEM_WOODEN_PLANKS] = (ItemRegistry){
		.name = "Wooden Planks",
	...
	reg[ITEM_WOOD_LOG] = (ItemRegistry){
		.name = "Wood Log",
	...
	reg[ITEM_BOUNCY_BLOCK] = (ItemRegistry){
		.name = "Bouncy Block",
	...
	reg[ITEM_ORANGE_WOOL] = (ItemRegistry){
		.name = "Orange Wool",
	...
	reg[ITEM_MAGENTA_WOOL] = (ItemRegistry){
		.name = "Magenta Wool",
	...
	reg[ITEM_LIGHT_BLUE_WOOL] = (ItemRegistry){
	...
	reg[ITEM_YELLOW_WOOL] = (ItemRegistry){
		.name = "Yellow Wool",
	...
	reg[ITEM_LIME_WOOL] = (ItemRegistry){
		.name = "Lime Wool",
	...
	reg[ITEM_PINK_WOOL] = (ItemRegistry){
		.name = "Pink Wool",
	...
	reg[ITEM_GRAY_WOOL] = (ItemRegistry){
		.name = "Gray Wool",
	...
	reg[ITEM_LIGHT_GRAY_WOOL] = (ItemRegistry){
	...
	reg[ITEM_CYAN_WOOL] = (ItemRegistry){
		.name = "Cyan Wool",
	...
	reg[ITEM_PURPLE_WOOL] = (ItemRegistry){
		.name = "Purple Wool",
	...
	reg[ITEM_BLUE_WOOL] = (ItemRegistry){
		.name = "Blue Wool",
	...
	reg[ITEM_BROWN_WOOL] = (ItemRegistry){
		.name = "Brown Wool",
	...
	reg[ITEM_GREEN_WOOL] = (ItemRegistry){
		.name = "Green Wool",
	...
	reg[ITEM_RED_WOOL] = (ItemRegistry){
		.name = "Red Wool",
	...
	reg[ITEM_BLACK_WOOL] = (ItemRegistry){
		.name = "Black Wool",
	...
	reg[ITEM_WOODEN_FENCE] = (ItemRegistry){
		.name = "Wooden Fence",
	...
	reg[ITEM_TRAPDOOR] = (ItemRegistry){
		.name = "Trapdoor",
	...
	reg[ITEM_WATER_BUCKET] = (ItemRegistry){
		.name = "Water Bucket",
	...
	reg[ITEM_SLAB_FRAME] = (ItemRegistry){
		.name = "Slab Frame",
	...
	reg[ITEM_STAIRS_FRAME] = (ItemRegistry){
		.name = "Stairs Frame",
	...
	reg[ITEM_NUB_FRAME] = (ItemRegistry){
		.name = "Nub Frame",
	...
	reg[ITEM_POWER_WIRE] = (ItemRegistry){
		.name = "Power Wire",
	...
	reg[ITEM_POWER_REPEATER] = (ItemRegistry) {
		.name = "Power Repeater",
		.atlas_idx = ATLAS_POWER_REPEATER,
		.model_idx = BLOCK_MODEL_QUAD,
		.blockId = BLOCK_POWER_REPEATER
	};

	reg[ITEM_POWERED_LAMP] = (ItemRegistry) {
		.name = "Powered Lamp",
		.atlas_idx = ATLAS_POWERED_LAMP,
		.model_idx = BLOCK_MODEL_QUAD,
		.blockId = BLOCK_POWERED_LAMP
	};

	for (int i = 0; i < ITEM_COUNT; i++) {
		ItemRegistry* irg = &reg[i];
		irg->mesh = (Mesh){ 0 };
...
void item_registry_free() {
	for (int i = 0; i < ITEM_COUNT; i++) { UnloadMesh(reg[i].mesh); }
	if (reg) free(reg);
}